...
human_index = "/scratch1/msc_2025/s2103976/reference_genomes/h_index_34/"

#Provide the path for the file containing the accession numbers
accession_list = "/scratch1/msc_2025/s2103976/hgps_se_accessions_34.txt"
    ...
    for accession in files_to_read:
        ...
        for doc in os.listdir(directory_of_files_to_align):
            if doc.startswith(accession) and doc.endswith("_trimmed.fastq.gz"):
                ...
                subprocess.run(f"STAR --runThreadN 10 --sjdbOverhang 34 --sjdbGTFfile {human_annot} --genomeDir {human_index} --readFilesIn {file_to_align} --readFilesCommand zcat --outSAMtype BAM SortedByCoordinate --outFileNamePrefix {output_prefix}", shell = True)
